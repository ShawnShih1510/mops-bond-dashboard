...
st.set_page_config(
    page_title="公開資訊觀測站 - 普通公司債歷年查詢與分析系統", layout="wide"
)
...
def fetch_single_year_market(year, market, params):
    ...
    try:
        resp = requests.post(
            url, data=payload, headers=headers, timeout=8, verify=False
        )
        resp.encoding = "utf-8"
        ...
        for r in rows:
            ...
            if len(cells) >= 10 and any(
                "公司債" in c or "新台幣" in c for c in cells
            ):
                while len(cells) < 13:
                    cells.append("")
...
def query_multi_year_engine(params):
    ...
    if not combined.empty:
        combined["發行總額_數值"] = (
            combined["發行總額"].astype(str).str.replace(",", "").str.strip()
        )
        combined["發行總額_數值"] = pd.to_numeric(
            combined["發行總額_數值"], errors="coerce"
        ).fillna(0)
        combined["票面利率_數值"] = pd.to_numeric(
            combined["票面利率"].astype(str).str.replace("%", "").str.strip(),
            errors="coerce",
        ).fillna(0)
        combined["月底餘額_數值"] = (
            combined["月底餘額"].astype(str).str.replace(",", "").str.strip()
        )
        combined["月底餘額_數值"] = pd.to_numeric(
            combined["月底餘額_數值"], errors="coerce"
        ).fillna(0)

        # 擷取發行年度供跨年統計
        combined["發行年度"] = combined["發行日期"].apply(
...
st.caption(
    "串接公開資訊觀測站 (MOPS) 普通公司債 ➜ 歷史資料查詢 (t120sb02_q5) | 支援 110"
    " ~ 115 年全量檢索"
)
...
if not raw_df.empty:
    ...
    # 3. 視覺化統計圖表（依查詢起訖年份動態顯示標題與億元單位）
    st.markdown("---")
    st.subheader("📊 3. 視覺化統計圖表")
    ch1, ch2 = st.columns(2)

    cur_start_y = st.session_state.get("curr_y1", 110)
    cur_end_y = st.session_state.get("curr_y2", 115)
